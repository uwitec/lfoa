<?php
// 分解单查询
class DecomposeStatisticsAction extends CommonAction
{
    private $defaultShowCols = array(
		array('name' => 'payPersonName', 'tableName' => 'order_base.payPerson as payPersonName', 'chName' => '付款人', 'colWidth' => '100'), 
    ...
    private function indexSearchList()
    {
        ...
        $CustomGoodsList = $CustomGoods->where($map)->distinct('payName')->field('payName')->select();
        $this->assign('CustomGoodsList', $CustomGoodsList);
        unset($map);
    ...
    private function indexSearch(&$map, &$searchStr)
    {
        ...
        if (isset($_REQUEST['payPersonName']) && !empty($_REQUEST['payPersonName'])) {
            $map['order_base.payPersonName'] = array(
                'eq',
                $_REQUEST['payPersonName']
            );
            $searchStr .= 'payPersonName/' . $_REQUEST['payPersonName'] . '/';
        }
        ...
        if (isset($_REQUEST['termMonth']) && !empty($_REQUEST['termMonth'])) {
            $BeginMonthTemp = strtotime($OrderYear.'-'.$_REQUEST['termMonth'].'-1');
			$endMonthTempAdd = $_REQUEST['termMonth'] + 1;
			$EndMonthTemp = strtotime($OrderYear.'-'.$endMonthTempAdd.'-1');
            $searchStr .= 'termMonth/' . $_REQUEST['termMonth'] . '/';
	...
	{
		$groupStr .= "order_base.payPerson, order_base.postCode, FROM_UNIXTIME( order_base.orderTime, '%m')";

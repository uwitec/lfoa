...
class ReturnRegisterAction extends CommonAction
{
    ...
    private function indexSearch(&$map, &$searchStr)
    {
        ...
        if ($_REQUEST['isReturnRegister'] && $_REQUEST['isReturnRegister'] == 1) {        	
            $map['return_order.quantity'] = array(
                'neq',
                'NULL'
            );
        }  
    ...
    private function getListFieldStr($colList, &$searchStr)
    {
        $fieldStr = '
			...
			return_order.memo as memo ,
			return_order.employeeID as employeeID ,
			return_order.insertDate   as insertDate  ,
			return_order.quantity as returnQuantity';
	...
	private function getConditionModel(&$model)
	{
		$model->table(array('tb_order_flow_details' => 'order_flow_details'))->
			...
			join('tb_return_order return_order on return_order.orderFlowID = order_flow_details.id');
	}
	...
	public function edit()
	{
		...
		if ($orderFlowIDStr)
		{
			$ReturnOrder = D('ReturnOrder');
			$row = array();

			$map['orderFlowID'] = array('in', $orderFlowIDStr);

			$list = $ReturnOrder->where($map)->select();
		...
		if ($orderFlowID)
		{
			...
			foreach($orderFlowID as $key=>$vo)
			{
				if(isset($id[$key]) && !empty($id[$key]))
				{
					...
					$data['orderFlowID'] = $vo;					
					$ReturnOrder->where($map)->data($data)->save();
				}
				else
				{
					$data['orderFlowID'] = $vo;				
					$ReturnOrder->data($data)->add();
				}
			}
